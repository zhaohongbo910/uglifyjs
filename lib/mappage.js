...
$(document).ready(function () {
  ...
  _mapMapper = 'viewer = new Cesium.Viewer(\
	...
	loadLayer();'; // leftmenu.update(1,"内容菜单");
  ...
  Sandcastle.addToolbarMenu(options); // newslist.ObjAbstract($("#Chart1"),1);
});
var Map = {
  'flytodst': function flytodst(itemid) {
    setLayout(0);
    $.ajax({
  ...
  //特殊：针对 APP接警记录
  'flytodst2': function flytodst2(itemid, location_x, location_y, time, name) {
    setLayout(0);
    $.ajax({
      url: 'http://47.240.13.42/xearth/earth-map/test',
      type: 'post',
  ...
  'flytodst3': function flytodst3(json) {
    var defaultCamera = 9000000.0;
...
var newslist = {
  ...
  'DisplayContent': function DisplayContent(title, content) {
    $('#modal-doc').remove();
    html = '<div id="modal-doc" class="modal fade">';
    html += '  <div class="modal-dialog" >';
    html += '    <div class="modal-content">';
  ...
  'pie': function pie(tabobj) {
    ...
    $.ajax({
      ...
      success: function success(json) {
        ...
        for (var i = 0; i < json.length; i++) {
          xdata.push({
            name: json[i].disname.substring(0, json[i].disname.indexOf('区域')),
            value: parseInt(json[i].gj) + parseInt(json[i].notgj)
          });
        } // console.log(xdata);
...
var show3DCoordinates = function show3DCoordinates() {
  var mapDivId; //地图底部工具栏显示地图坐标信息
  ...
  if (coordinatesDiv) {
    ...
    coordinatesDiv.innerHTML = "<span id='cd_label' style='font-size:13px;text-align:center;font-family:微软雅黑;color:#edffff;'>暂无坐标信息</span>"; //document.getElementById(this.mapDivId).appendChild(coordinatesDiv);
    ...
    handler3D.setInputAction(function (movement) {
      ...
      if (pick) {
        ...
        if (cartesian) {
          ...
          if (cartographic) {
            //海拔
            var height = viewer.scene.globe.getHeight(cartographic); //视角海拔高度
            ...
            var he2 = Math.sqrt(cartesian.x * cartesian.x + cartesian.y * cartesian.y + cartesian.z * cartesian.z); //地理坐标（弧度）转经纬度坐标
...
var showAroundResourceForEarth = function showAroundResourceForEarth(name, longitude, latitude, telephone, img, description) {
  viewer.entities.add({
    name: name,
